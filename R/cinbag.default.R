...
# randomForest object with an additional output, a matrix with inbag 
# counts (rows) for each tree (columns).  rfobj$inbagCount is similar 
# to rfobj$inbag, but with inbag counts instead of inbag indicators.
# ----------------------------------------------------------------


## mylevels() returns levels if given a factor, otherwise 0. cinbag uses this function.
...
"cinbag" <-
    ...
    if (classRF) {
        ...
        out <- list(call = cl,
                    ...
                    # Added code
                    # --------------------------------------------------
                    inbagCount = if (keep.inbag) rfout$inbagCount else NULL
                    # --------------------------------------------------
                    )
    } else {
		ymean <- mean(y)

...
void classRFmod(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	     ...
	     int *outclts, int *labelts, double *proxts, double *errts,
             int *inbag, int *inbagCount /*** Added code ***/) {
               
/*** Note: the name of this function, classRFmod, has been changed from the original classRF. ***/               
	...
	*at, *a, *b, *mind, *nind, *jts, *oobpair;
    ...
    jinCount =      (int *) S_alloc(nsample, sizeof(int)); /*** Added code ***/
    ...
    for (jb = 0; jb < Ntree; jb++) {
		...
		do {
			...
			if (stratify) {  /* stratified sampling */
				zeroInt(jin, nsample);
        zeroInt(jinCount, nsample); /*** Added code ***/
				...
				if (replace) {  /* with replacement */
					for (n = 0; n < nstrata; ++n) {
						for (j = 0; j < sampsize[n]; ++j) {
							...
							jin[k] = 1;
              jinCount[k] = jinCount[k] + 1; /*** Added code ***/
						}
					}
          ...
          zeroInt(jinCount, nsample); /*** Added code ***/
            ...
            if (keepInbag) {
                for (n = 0; n < nsample0; ++n) {
                    ...
                    inbagCount[n + idxByNsample] = jinCount[n]; /*** Added code ***/
                }
            }
